...
public interface AppraisalsRepos extends JpaRepository<Appraisals, Integer>{
	...
	@Query("from User u where u.active=?1 and u.internal=?2  and u.affectation.hrManager=?3 and u.username in (select r.user.username from UserRole r where r.user.username=u.username and r.role='ROLE_MYPM') and u.username not in (select up.employ.username from UserAppraisal up where up.appraisal=?4)")
