...
public interface UserAppraisalRepos extends JpaRepository<UserAppraisal, Integer>{
	...
	@Query("from UserAppraisal u where u.appraisal=?1 and u.appraisee=?2 ")
	List<UserAppraisal> findByAppraisalAndManager(Appraisals ap,User user);
	
	@Query("from UserAppraisal u where u.employ=?1 ")
	List<UserAppraisal> findUserAppraisalByUser(User user);
